...
const LinkedImageComponent: React.FC<ReactNodeViewProps> = ({ node }) => {
  const src = node.attrs.src
  const href = node.attrs.href
  const alt = node.attrs.alt || 'Image'
  const title = node.attrs.title || ''
  const isGithubBadge = node.attrs['data-github-badge'] === 'true'
  const isImageLink = node.attrs['data-image-link'] === 'true'
  ...
  useEffect(() => {
    ...
    const processImageSrc = async () => {
      console.log('🖼️ Processing linked image source:', src)
  ...
  // badge 样式
  const badgeStyle = (isGithubBadge || isImageLink) ? {
    height: '20px',
    maxWidth: 'none',
    ...
    <NodeViewWrapper>
      {href ? (
        <a
          ...
          data-github-badge={isGithubBadge ? 'true' : undefined}
